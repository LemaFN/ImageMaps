...
Mit Auto-Place: Du bekommst nur **eine einzige Karte** (oben-links) ins Inventar. Baue einen
leeren Item Frame an eine gerade Wand (Nord/Süd/Ost/West, kein Boden/Decke) und setze diese eine
Karte hinein. Das Plugin erkennt das automatisch und ergänzt die restlichen Karten selbstständig
nach rechts und unten, ausgehend von Position und Blickrichtung dieses Frames. Kein zweiter
Command nötig. Klappt die Platzierung nicht (z.B. Boden/Decke), bekommst du eine Meldung und
die restlichen Karten landen stattdessen im Inventar.
...
## Falls "Fehler bei Bildverarbeitung" bei PNG-Dateien auftritt

Ich konnte diesen Fehler mit vielen realistischen Test-PNGs (verschiedene Größen, Farbtiefen,
mit/ohne Alphakanal, indiziert, Graustufen, bis 18 MB) nicht reproduzieren. Falls er trotzdem
nochmal auftritt: Die Fehlerseite und die Server-Konsole zeigen jetzt zusätzlich die erkannte
Datei-Signatur an.

- Steht dort **"PNG-Signatur korrekt erkannt ... evtl. abgeschnitten/beschädigt"** → die Datei
  kam beim Server tatsächlich beschädigt/unvollständig an (z.B. Netzwerkproblem). Bitte den
  genauen Bytecount und die Server-Log-Zeile "Content-Length stimmt nicht..." (falls vorhanden)
  mitschicken.
- Steht dort **"unbekannte Signatur, erste Bytes: ..."** → das Datei-Feld wurde beim
  Multipart-Parsing falsch extrahiert. Bitte diese Bytes mitschicken, das identifiziert den
  Bug sofort.

Damit lässt sich der Fehler beim nächsten Auftreten gezielt fixen, statt weiter zu raten.
