...
public class UploadServer {
    ...
    private class UploadHandler implements HttpHandler {
        ...
        private void handlePost(HttpExchange exchange) throws IOException {
            ...
            byte[] body = readAllLimited(exchange.getRequestBody(), maxBytes, declaredLength);
            if (body == null) {
            ...
            plugin.getLogger().fine("Upload empfangen: body=" + body.length + " bytes, boundary-Länge=" + boundary.length()
                    + ", " + parts.size() + " Teile geparst, Datei-Feld='" + uploadedFilename + "' ("
                    + (imageBytes == null ? "null" : imageBytes.length + " bytes") + ")");
            ...
            try {
                ...
                plugin.getLogger().log(Level.WARNING, "Bildverarbeitung fehlgeschlagen für Datei '" + uploadedFilename
                        + "' (" + imageBytes.length + " bytes, erste Bytes: " + hexPrefix(imageBytes, 16) + ")", e);
                respond(exchange, 400, "text/html; charset=utf-8", htmlPage("Fehler bei Bildverarbeitung",
                        ...
                        + "<p><small>Debug-Info (auch in der Server-Konsole geloggt): " + imageBytes.length
                        + " Bytes empfangen, Datei: " + escape(uploadedFilename)
                        + ", erste Bytes: " + hexPrefix(imageBytes, 16) + "</small></p>"));
                return;
            }
    ...
    private byte[] readAllLimited(java.io.InputStream in, int maxBytes, String declaredContentLength) throws IOException {
        ...
        if (declaredContentLength != null) {
            try {
                long declared = Long.parseLong(declaredContentLength);
                if (declared != total) {
                    plugin.getLogger().warning("Content-Length stimmt nicht mit tatsächlich gelesenen Bytes überein! "
                            + "Deklariert=" + declared + ", gelesen=" + total
                            + " - das würde auf ein abgeschnittenes Upload hindeuten.");
                }
    ...
    private String hexPrefix(byte[] data, int len) {
